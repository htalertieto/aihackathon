...
const LANGUAGE_LOCALES = {
  Arabic: 'ar-SA', English: 'en-US', French: 'fr-FR', German: 'de-DE', Hindi: 'hi-IN',
  Italian: 'it-IT', Korean: 'ko-KR', 'Mandarin Chinese': 'zh-CN', Polish: 'pl-PL',
  Portuguese: 'pt-PT', Russian: 'ru-RU', Spanish: 'es-ES', Tagalog: 'fil-PH',
...
export default function ChatPanel({ patientLanguage }) {
  ...
  function SpeakButton({ text, language, label }) {
    return (
      <button type="button" className="speak-button" onClick={() => speak(text, language)} aria-label={label} title="Listen">
  ...
  return (
    <section className="panel chat-panel" aria-label="Doctor-patient communication">
      <div className="chat-header">
        <div className="chat-title">
          <span className="icon-disc"><PaperAirplaneIcon aria-hidden="true" /></span>
          <div>
            <h2>Doctor-Patient Communication</h2>
            <p><LockClosedIcon aria-hidden="true" /> Secure conversation</p>
          </div>
        </div>
        {messages.length > 0 && <button type="button" className="secondary" onClick={startNewChat} disabled={loading}>New chat</button>}
      </div>

      <div className="chat-messages" aria-live="polite">
        {messages.length === 0 && <p className="chat-empty">Write the first doctor message to detect its language.</p>}
        {messages.map((message) => (
          <article className={`chat-message ${message.speaker.toLowerCase()}`} key={message.id}>
            ...
            <div className="message-text-row">
              ...
              <SpeakButton text={message.originalText} language={message.originalLanguage} label={`Listen to ${message.speaker} message in ${message.originalLanguage}`} />
            </div>
            <div className="chat-translation">
              <strong>Translation - {message.translatedLanguage}</strong>
              <div className="message-text-row">
                <p>{message.translatedText}</p>
                <SpeakButton text={message.translatedText} language={message.translatedLanguage} label={`Listen to translation in ${message.translatedLanguage}`} />
              </div>
            </div>
          </article>
        ))}
      ...
      <form onSubmit={passTurn} className="chat-compose">
        ...
        <textarea id="chat-message" rows={4} value={draft} onChange={(event) => setDraft(event.target.value)} placeholder={isUserTurn ? 'Write what you want to say to the patient...' : 'Write the patient response...'} disabled={loading} />
      ...
      <p className="chat-security"><ShieldCheckIcon aria-hidden="true" /> Your conversation is private and secure.</p>
    </section>
  );
}
